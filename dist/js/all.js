...
;(function($){
	...
	$.fn.bxSlider = function(options){
		...
		var appendControls = function(){
			slider.controls.next = $('<a class="bx-next icon-arrow-right" href="">' + slider.settings.nextText + '</a>');
			slider.controls.prev = $('<a class="bx-prev icon-arrow-left" href="">' + slider.settings.prevText + '</a>');
...
$(document).ready(function() {
		...
		$(".slider ul").bxSlider({
			...
			onSliderLoad: function(currentIndex) {
				...
				TweenMax.to(element, 0.4, {
					...
					ease: Power2.easeOut,
					onComplete: function() {
						TweenMax.to(element.find(".read-more"), 0.4, {
							autoAlpha: 1,
							scale: 1,
							ease: Power2.easeOut
						});
			...
			onSlideBefore: function($slideElement, oldIndex, newIndex) {
				var element = $(".slider li").eq(oldIndex).find(".caption");
				TweenMax.to(element.find(".read-more"), 0.4, {
					autoAlpha: 0,
					scale: 0.8,
					ease: Power2.easeOut
			...
			onSlideAfter: function($slideElement, oldIndex, newIndex) {
				TweenMax.to($slideElement.find(".caption"), 0.4, {
					scale: 1,
					autoAlpha: 1,
					ease: Power2.easeOut
				});
				TweenMax.to($slideElement.find(".read-more"), 0.4, {
					autoAlpha: 1,
					scale: 1,
					ease: Power2.easeOut,
					delay: 0.2
	...
	if ($(".gallery-slider").length > 0) {
		$(".gallery-slider > ul").bxSlider({
			...
			controls: true,
			onSliderLoad: function(currentIndex) {
				var totalItems = $(".gallery-slider .bx-wrapper li").length;
				$(".pager-total").text(minTwoDigits(totalItems));
			},
			onSlideAfter: function($slideElement, oldIndex, newIndex) {
				$(".pager-active").text(minTwoDigits(newIndex + 1));
			}
		});
	}

	// Make input full width in subscribe small on focus
	$(".subscribe-small input").focus(function() {
		$(".subscribe-small").addClass("is-focused");
	});

	// Make input smaller in subscribe small on blur
	$(".subscribe-small input").blur(function() {
		$(".subscribe-small").removeClass("is-focused");
	});
		...
		$(".search").fadeIn("slow", function() {
			TweenMax.to($(".search-header"), 1, {
				...
				onComplete: function() {
					...
					TweenMax.staggerTo($(".search-content li"), 1, {
						autoAlpha: 1,
						delay: 0.6
					}, 0.15);
				}
	...
	// Isotope init for article filtering
	if ($(".filter").length > 0) {
...
$(window).load(function() {
	...
	// Load animation in Home start
	if ($(".loader").length > 0) {
		TweenMax.to($(".logo-1"), 2, {
			autoAlpha: 1,
			scale: 1,
		...
		TweenMax.to($(".logo-2"), 1, {
			autoAlpha: 1,
			width: "100%",
			delay: 1.6,
			ease: Power2.easeInOut,
			onComplete: function() {
				TweenMax.to($(".logo-loader"), 0.8, {
					autoAlpha: 0,
					delay: 1
				});
				TweenMax.to($(".loader"), 1.2, {
					height: 0,
					autoAlpha: 0,
					delay: 1.5
				});
